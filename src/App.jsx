...
import { CheckCircle2, XCircle, Clock, Users, Target, PlusCircle, Filter, ChevronDown, FileDown } from 'lucide-react';

// --- DADOS MOCADOS (Simulando uma API) ---
const user = {
    name: 'Thiago Almeida',
...
const ProgressBar = ({ value }) => {
    ...
    return (
        <div className="w-full bg-gray-200 rounded-full h-2.5">
            <div className={`${colorClass} h-2.5 rounded-full`} style={{ width: `${cappedValue}%` }}></div>
        </div>
    );
};
...
const KrTableRow = ({ kr }) => {
    ...
    return (
        <tr className="border-b border-gray-200 hover:bg-gray-50">
            ...
            <td className="py-4 px-6">
                <span className={`inline-flex items-center space-x-1.5 py-1 px-3 rounded-full text-xs font-medium ${statusInfo.text} bg-gray-100`}>
                    {statusInfo.icon}
                    <span>{kr.status}</span>
                </span>
            </td>
            ...
            <td className="py-4 px-6">
                <button className="text-blue-600 hover:underline font-semibold">Detalhes</button>
            </td>
        </tr>
    );
};
...
const KrTable = ({ krs }) => (
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
            ...
            <div className="flex items-center space-x-2">
                <button className="flex items-center space-x-2 bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-100 transition">
                    <Filter size={16} />
                    <span>Filtrar</span>
                </button>
                <button className="flex items-center space-x-2 bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-100 transition">
        ...
        <div className="overflow-x-auto">
            <table className="w-full text-sm text-left text-gray-500">
                <thead className="text-xs text-gray-700 uppercase bg-gray-100">
                    <tr>
                        <th scope="col" className="py-3 px-6">Key Result</th>
                        <th scope="col" className="py-3 px-6">Responsável</th>
                        <th scope="col" className="py-3 px-6">Progresso</th>
                        <th scope="col" className="py-3 px-6">Status</th>
                        <th scope="col" className="py-3 px-6">Prazo Final</th>
                        <th scope="col" className="py-3 px-6">Ações</th>
                    </tr>
                </thead>
                <tbody>
                    {krs.map(kr => <KrTableRow key={kr.id} kr={kr} />)}
                </tbody>
            </table>
        </div>
        
        <div className="p-4 border-t border-gray-200 flex justify-between items-center text-sm">
            <span className="text-gray-600">Mostrando 1-5 de 5 resultados</span>
            <div className="flex space-x-1">
                <button className="px-3 py-1 border rounded-md bg-gray-100">Anterior</button>
                <button className="px-3 py-1 border rounded-md bg-blue-500 text-white">1</button>
                <button className="px-3 py-1 border rounded-md bg-gray-100">Próximo</button>
            </div>
        </div>
    </div>
);
